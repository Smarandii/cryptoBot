...
class UserMenu:
    ...
    def suggestion_menu(self):
        keyboard = types.InlineKeyboardMarkup(row_width=1)
        group_btn = types.InlineKeyboardButton(text="Наш групповой чат 💬", url=self.URLS['group'])
        channel_btn = types.InlineKeyboardButton(text="Наш канал 📢", url=self.URLS['channel'])
        keyboard.add(group_btn, channel_btn)
        return keyboard

    def sent_by_start_menu(self, text):
        if text in self.MENU_BUTTONS.values():
            return True
        return False
...
class OperatorMenu:
    MENU_BUTTONS = {'n_a_requests': 'Показать заявки, требующие обработки',
